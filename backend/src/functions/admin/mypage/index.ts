import { AccountForm, AccountFormSchema, MypageFormOfIndividualUser, ScoreItemDtoSchema, TeamFormSchema } from "@scobit/types";
import { APIGatewayProxyEvent, APIGatewayProxyEventV2 } from "aws-lambda";
import { event } from "local/events/newTeamEvent";
...
export const mypageFetchTeams = async (event: APIGatewayProxyEvent): Promise<ResponseBodyBuilder> => {
  ...
  try {
    const teams = await TeamService.findTeamsByAccountId(account_id, client);
...
export const editAccount = async (event: APIGatewayProxyEvent): Promise<ResponseBodyBuilder> => {
  logger.info('チーム選択処理開始。');
  const payload = JwtUtil.checkJwtAndGetPayload(event);
  ...
  const valid = AccountFormSchema.safeParse(account);
  if(!valid.success){
    return ResponseUtil.error().addErrors(convertToErrorInfos(valid.error));
  }
  const checkedAccount = valid.data;

  const client = await getPool().connect();
  try {
    const cnt1 = await AccountService.getCountForPublicIdExcludeAccountId(checkedAccount.account_pub_id, payload.sub, client);
    logger.debug(`重複しているアカウント数:${cnt1}`)
    if(cnt1 > 0){
      return ResponseUtil.error().addError('account_pub_id', '入力されたアカウントIDは使用できません。');
    }
    const cnt2 = await AccountService.getCountForEmailExcludeAccountId(checkedAccount.email, payload.sub, client);
    logger.debug(`重複しているメールアドレス数:${cnt2}`)
    if(cnt2 > 0){
      return ResponseUtil.error().addError('email', '入力されたメールアドレスはすでに使用されています。');
    }

    const account = await AccountService.updateAccount(checkedAccount, payload.sub, client);

    return ResponseUtil.success().putData('account', account);
